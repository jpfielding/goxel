...
type MRImage struct {
	...
	// Compression codec (nil = uncompressed)
	Codec Codec
}
...
func (mr *MRImage) GetDataset() (*Dataset, error) {
	...
	if mr.Codec != nil {
		ts = mr.Codec.TransferSyntaxUID()
	} else if mr.PixelData != nil && mr.PixelData.IsEncapsulated {
		// Already encapsulated data - default to JPEG-LS
	...
	// 7. Pixel Data
	if mr.Codec != nil && mr.PixelData != nil && !mr.PixelData.IsEncapsulated {
		flatData := mr.PixelData.GetFlatData()
		opts = append(opts, WithPixelData(mr.Rows, mr.Columns, int(mr.BitsAllocated), flatData, mr.Codec))
...
func (mr *MRImage) Write(path string) (int64, error) {
	slog.Debug("Writing MR DICOM file", "path", path, "frames", mr.NumberOfFrames, "compressed", mr.Codec != nil)
