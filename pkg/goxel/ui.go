...
func labeledRow(label string, content fyne.CanvasObject) *fyne.Container {
...
func (v *Viewer) createSideBySidePanel() fyne.CanvasObject {
	...
	sliceDebounce := newDebouncer(50 * time.Millisecond)
	sliceControl.SetOnChanged(func(val float64) {
		idx := int(val)
		sliceDebounce.call(func() {
			fyne.Do(func() {
				v.setFrame(idx)
	...
	wlDebounce := newDebouncer(50 * time.Millisecond)
	wlEntry.OnChanged = func(s string) {
		val, _ := strconv.Atoi(s)
		valCopy := val
		wlDebounce.call(func() {
			fyne.Do(func() {
				v.dicox.SetWindowLevel(valCopy)
				v.image.Refresh()
			})
		})
	}
	...
	wwDebounce := newDebouncer(50 * time.Millisecond)
	wwEntry.OnChanged = func(s string) {
		val, _ := strconv.Atoi(s)
		valCopy := val
		wwDebounce.call(func() {
			fyne.Do(func() {
				v.dicox.SetWindowWidth(valCopy)
				v.image.Refresh()
			})
		})
	}
	...
	materialDebounce := newDebouncer(50 * time.Millisecond)
	materialSlider.OnChanged = func(thresholds []int) {
		thresholdsCopy := append([]int(nil), thresholds...)
		bands := append([]volume.ColorBand(nil), materialSlider.Bands...)
		maxVal := materialSlider.Max
		materialDebounce.call(func() {
			fyne.Do(func() {
				if v.volumeRenderer != nil {
					// First threshold controls density cutoff (Air band)
					if len(thresholdsCopy) > 0 && len(bands) > 0 && bands[0].IsTransparent {
						normalized := float64(thresholdsCopy[0]) / maxVal
						v.volumeRenderer.SetDensityThreshold(normalized)
					}
					v.volumeRenderer.SetMaterialBands(bands)
	...
	// Preset
	presetSelect := widget.NewSelect([]string{"DEFAULT", "FINDING", "MONOCHROME", "LAPTOP_REMOVAL"}, func(selected string) {
	...
	opacityDebounce := newDebouncer(50 * time.Millisecond)
	opacitySlider.SetOnChanged(func(val float64) {
		valCopy := val
		opacityDebounce.call(func() {
			fyne.Do(func() {
				// Update 3D view
				v.volumeRenderer.SetAlphaScale(valCopy)
				v.volumeRenderer.Render()
				v.volumeRenderer.Refresh()
				// Sync to 2D composite view
				if v.dicox != nil {
					v.dicox.SetAlphaScale(valCopy)
					if v.image != nil {
						v.image.Refresh()
					}
				}
	...
	zScaleDebounce := newDebouncer(50 * time.Millisecond)
	v.zScaleSlider.SetOnChanged(func(val float64) {
		valCopy := val
		v.currentScaleZ = valCopy
		zScaleDebounce.call(func() {
			fyne.Do(func() {
				v.volumeRenderer.SetScaleZ(valCopy)
				v.volumeRenderer.Render()
				v.volumeRenderer.Refresh()
			})
		})
	...
	bandOpacityDebounce := newDebouncer(50 * time.Millisecond)
	materialSlider.OnOpacityChanged = func(_ int, _ float64) {
		bands := append([]volume.ColorBand(nil), materialSlider.Bands...)
		bandOpacityDebounce.call(func() {
			fyne.Do(func() {
	...
	bgColorBtn := widget.NewButton("BG Color", func() {
		...
		bgDebounce := newDebouncer(50 * time.Millisecond)
		updateColor := func(r, g, b uint8) {
			c := color.RGBA{R: r, G: g, B: b, A: 255}
			bgDebounce.call(func() {
				fyne.Do(func() {
		...
		// Bind Callbacks
		rSlider.SetOnChanged(func(_ float64) {
			updateColor(uint8(rSlider.Slider.Value), uint8(gSlider.Slider.Value), uint8(bSlider.Slider.Value))
		})
		gSlider.SetOnChanged(func(_ float64) {
			updateColor(uint8(rSlider.Slider.Value), uint8(gSlider.Slider.Value), uint8(bSlider.Slider.Value))
		})
		bSlider.SetOnChanged(func(_ float64) {
			updateColor(uint8(rSlider.Slider.Value), uint8(gSlider.Slider.Value), uint8(bSlider.Slider.Value))
		})

		// Layout
		content := container.NewVBox(
			labeledRow("Red", rSlider),
			labeledRow("Green", gSlider),
			labeledRow("Blue", bSlider),
		)
	...
	globalControls := container.NewVBox(
		labeledRow("Quality:", qualitySelect),
		labeledRow("Preset:", presetSelect),
		labeledRow("Opacity:", opacitySlider),
		labeledRow("Z Scale:", v.zScaleSlider),
	...
	// Volume Selection
	volRow := labeledRow("Volume:", volumeSelect)

	// View Orientation
	viewRow := labeledRow("View:", orientSelect)
	...
	wlRow := container.NewGridWithColumns(2,
		labeledRow("W/L:", wlEntry),
		labeledRow("W/W:", wwEntry),
	)

	// Slice Slider
	sliceRow := labeledRow("Slice:", sliceControl)
	...
	for _, k := range volKeys {
		...
		if isFinding {
			...
			slider.OnChanged = func(val float64) {
				valCopy := val
				vol.Alpha = valCopy
				valLabel.SetText(fmt.Sprintf("%.2f", valCopy))
				alphaDebounce.call(func() {
					fyne.Do(func() {
						v.setup3DView()
					})
