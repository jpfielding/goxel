...
func (vr *CPUVolumeRenderer) Render() {
	...
	// Ray marching parameters - use configured step size
	stepSize := vr.StepSize
	vr.Mu.RUnlock() // Release read lock now that we have stepSize
	vr.Mu.RLock()   // Re-acquire for consistency check
	_ = stepSize    // Suppress warning
	stepSize = vr.StepSize
	vr.Mu.RUnlock()
	windowMin := wl - ww/2
	windowMax := wl + ww/2
						...
						if pos.X >= 0 && pos.X < 1 && pos.Y >= 0 && pos.Y < 1 && pos.Z >= 0 && pos.Z < scaleZ {
							...
							if density > 0 {
								...
								if compositeMode {
									...
									// Apply window/level to HE for alpha
