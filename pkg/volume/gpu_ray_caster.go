...
void main() {
    ...
    for (int i = 0; i < 4096; i++) {
        ...
        if (normalized > densityThreshold) {
            ...
            // Gradient-based lighting with finer sampling for smoother normals
            float eps = 0.003;
            float dx = texture(volumeTex, texPos + vec3(eps, 0.0, 0.0)).r - texture(volumeTex, texPos - vec3(eps, 0.0, 0.0)).r;
            float dy = texture(volumeTex, texPos + vec3(0.0, eps, 0.0)).r - texture(volumeTex, texPos - vec3(0.0, eps, 0.0)).r;
            float dz = texture(volumeTex, texPos + vec3(0.0, 0.0, eps)).r - texture(volumeTex, texPos - vec3(0.0, 0.0, eps)).r;
...
type GPUVolumeRenderer struct {
	...
	// Center of mass for rotation pivot (normalized 0-1 coordinates)
	centerOfMassX, centerOfMassY, centerOfMassZ float32
...
// Valid presets: "DEFAULT", "THREAT", "MONOCHROME", "LAPTOP_REMOVAL"
...
// computeCenterOfMass calculates the density-weighted centroid of volume data
// Returns normalized coordinates (0-1 range) for x, y, z
func computeCenterOfMass(data []uint16, dimX, dimY, dimZ int, densityThreshold uint16) (float32, float32, float32) {
		...
		for y := 0; y < dimY; y++ {
			for x := 0; x < dimX; x++ {
				...
				if density < densityThreshold {
					continue // Skip low-density voxels (air, background)
				}
				weight := float64(density)
				sumX += float64(x) * weight
				sumY += float64(y) * weight
				sumZ += float64(z) * weight
				totalWeight += weight
			}
		}
	}
	...
	if totalWeight == 0 {
		// Fallback to geometric center if no significant density
		return 0.5, 0.5, 0.5
	}

	// Normalize to 0-1 range
	centerX := float32(sumX/totalWeight) / float32(dimX)
	centerY := float32(sumY/totalWeight) / float32(dimY)
	centerZ := float32(sumZ/totalWeight) / float32(dimZ)
...
func (gr *GPUVolumeRenderer) AddVolume(pd *PixelData, imageRows, imageCols int, pixelRep int, tint color.RGBA, wl, ww, intercept, alphaScale float64) error {
	...
	slog.Info("GPU AddVolume", "dim", fmt.Sprintf("%dx%dx%d", dimX, dimY, dimZ), "range", fmt.Sprintf("%d-%d", minVal, maxVal), "avg", avg, "wl", wl, "ww", ww)

	// Compute center of mass for first volume (primary)
	if len(gr.volumes) == 0 {
		// Use threshold based on density range (skip bottom 10% as air/background)
		threshold := uint16(float64(maxVal-minVal)*0.1 + float64(minVal))
		comX, comY, comZ := computeCenterOfMass(data, dimX, dimY, dimZ, threshold)
		gr.centerOfMassX = comX
		gr.centerOfMassY = comY
		gr.centerOfMassZ = comZ
		gr.hasCenterOfMass = true
		slog.Info("Center of mass computed", "x", comX, "y", comY, "z", comZ)
	}
...
// uploadVolume uploads a single volume to GPU
func (gr *GPUVolumeRenderer) uploadVolume(vol *ActiveVolume) {
	if len(vol.Data) == 0 {
		return
	}

	// Convert uint16 to normalized float32
	data := make([]float32, len(vol.Data))
	...
	gl.TexImage3D(gl.TEXTURE_3D, 0, gl.R32F,
		int32(vol.DimX), int32(vol.DimY), int32(vol.DimZ),
		0, gl.RED, gl.FLOAT, gl.Ptr(data))
...
func (gr *GPUVolumeRenderer) renderInternal() *image.RGBA {
	...
	hasCOM := gr.hasCenterOfMass
	comX, comY, comZ := gr.centerOfMassX, gr.centerOfMassY, gr.centerOfMassZ
	...
	// Use center of mass as rotation pivot if computed, else geometric center
	var centerX, centerY, centerZ float32
	if hasCOM {
		centerX, centerY, centerZ = comX, comY, comZ*scaleZ
	} else {
		centerX, centerY, centerZ = 0.5, 0.5, scaleZ*0.5
	}
	...
	// Draw threats moved to end to prevent state pollution
	...
	gl.Uniform3f(gl.GetUniformLocation(gr.program, gl.Str("camPos\x00")), camX, camY, camZ)
	gl.Uniform3f(gl.GetUniformLocation(gr.program, gl.Str("camForward\x00")), fwdX, fwdY, fwdZ)
	gl.Uniform3f(gl.GetUniformLocation(gr.program, gl.Str("camRight\x00")), rightX, rightY, rightZ)
	gl.Uniform3f(gl.GetUniformLocation(gr.program, gl.Str("camUp\x00")), upX, upY, upZ)
	gl.Uniform1f(gl.GetUniformLocation(gr.program, gl.Str("fov\x00")), 0.8)
	gl.Uniform1f(gl.GetUniformLocation(gr.program, gl.Str("aspectRatio\x00")), float32(width)/float32(height))
	gl.Uniform1f(gl.GetUniformLocation(gr.program, gl.Str("stepSize\x00")), 0.001)
	gl.Uniform1f(gl.GetUniformLocation(gr.program, gl.Str("scaleZ\x00")), scaleZ)
	gl.Uniform1f(gl.GetUniformLocation(gr.program, gl.Str("ambientIntensity\x00")), float32(gr.ambientIntensity))
	gl.Uniform1f(gl.GetUniformLocation(gr.program, gl.Str("diffuseIntensity\x00")), float32(gr.diffuseIntensity))
	gl.Uniform1f(gl.GetUniformLocation(gr.program, gl.Str("specularIntensity\x00")), float32(gr.specularIntensity))

	// Clipping
	gl.Uniform4f(gl.GetUniformLocation(gr.program, gl.Str("clipPlane\x00")), gr.clipPlane[0], gr.clipPlane[1], gr.clipPlane[2], gr.clipPlane[3])
	if gr.enableClip {
		gl.Uniform1i(gl.GetUniformLocation(gr.program, gl.Str("enableClip\x00")), 1)
	} else {
		gl.Uniform1i(gl.GetUniformLocation(gr.program, gl.Str("enableClip\x00")), 0)
	}

	// Set Transfer Function Texture
	gl.ActiveTexture(gl.TEXTURE1)
	gl.BindTexture(gl.TEXTURE_1D, gr.transferTex)
	gl.Uniform1i(gl.GetUniformLocation(gr.program, gl.Str("transferTex\x00")), 1)
	...
	for _, vol := range volumesToRender {
		...
		gl.Uniform1f(gl.GetUniformLocation(gr.program, gl.Str("windowMin\x00")), wl-ww/2)
		gl.Uniform1f(gl.GetUniformLocation(gr.program, gl.Str("windowRange\x00")), ww)
		gl.Uniform1f(gl.GetUniformLocation(gr.program, gl.Str("alphaScale\x00")), volAlpha)
		gl.Uniform1f(gl.GetUniformLocation(gr.program, gl.Str("rescaleIntercept\x00")), intercept)
		gl.Uniform1f(gl.GetUniformLocation(gr.program, gl.Str("densityThreshold\x00")), float32(gr.densityThreshold))

		// Set Tint Color
		gl.Uniform4f(gl.GetUniformLocation(gr.program, gl.Str("tintColor\x00")),
			float32(vol.Color.R)/255.0, float32(vol.Color.G)/255.0, float32(vol.Color.B)/255.0, float32(vol.Color.A)/255.0)

		// Bind Volume Texture
		gl.ActiveTexture(gl.TEXTURE0)
		gl.BindTexture(gl.TEXTURE_3D, vol.Texture)
		gl.Uniform1i(gl.GetUniformLocation(gr.program, gl.Str("volumeTex\x00")), 0)
...
// AddFinding adds a threat bounding box to the display
...
// SetFindingMode sets the threat display mode
...
func (gr *GPUVolumeRenderer) drawFindings(mvp [16]float32) {
	...
	mvpLoc := gl.GetUniformLocation(gr.lineProgram, gl.Str("mvp\x00"))
	if mvpLoc == -1 {
